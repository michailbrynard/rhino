...
export const signup = (signup_email) => (
	dispatch => {
		...
		callApi('POST', route, null, { signup_email, company: process.env.REACT_APP_COMPANY_IDENTIFIER, referral_id: "1234" })
		.then(json => {
			if (json.status === 'success') {
				dispatch({ type: SIGNUP_SUCCESS, data: json.data })
			} else {
				dispatch({ type: SIGNUP_ERROR, err: json.data[0] || json.message })
			}
		})
		.catch(err => {
		...
		callApi('POST', route, null, { new_password1, new_password2, uid, token })
			.then(json => {
				if (json.status === 'success') {
					...
					callApi('POST', route, null, { user: email, password: new_password1, company: process.env.REACT_APP_COMPANY_IDENTIFIER })
						.then(json => {
							if (json.status === 'success') {
...
export const login = (user, password) => (
	dispatch => {
		...
		callApi('POST', route, null, { user, password, company: process.env.REACT_APP_COMPANY_IDENTIFIER })
			.then(json => {
				if (json.status === 'success') {
						dispatch({ type: LOGIN_SUCCESS, data: json.data })
				} else {
